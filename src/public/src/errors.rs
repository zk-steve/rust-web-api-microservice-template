...
mod tests {
    ...
    async fn test_return_error_not_found() {
        let rejection = warp::reject::custom(WarpError(Error::NotFound));
    ...
    async fn test_return_error_parse_error() {
        ...
        let rejection = warp::reject::custom(WarpError(Error::ParseError(parse_error)));
        let response = return_error(rejection).await.unwrap().into_response();
    ...
    async fn test_return_error_missing_parameters() {
        let rejection = warp::reject::custom(WarpError(Error::MissingParameters));
        let response = return_error(rejection).await.unwrap().into_response();
